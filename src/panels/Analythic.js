...
const Analythic = ({id,setActivePanel,protocol_,hashes}) =>{
    ...
    const fetchData = async () =>{
        let content = await fetch(`${protocol_}://${hashes.find(x=>x.name === "d").value}/crm/php/vk/wbh_vk.php?menu=3`);
        ...
        <View activePanel={id} popout={popout}>
            <Panel id={id}>
                <Div className={"div_d"}>
                    ...
                    <Title style={{alignSelf:"start",fontWeight:1000}} level={1}>Аналитика партнера</Title>
                    <div className={"d_dz"}>
                        {ants && ants.length === 0 &&
                            <Placeholder
